...
<section class="team-tab press-con section-padding">
    ...
    @foreach ($items as $index => $item)
        <div id="tab-{{ $loop->iteration }}"
             ...
             style="
                background-image: url('{{ asset('frontend/imgs/submenu/' . $item->image) }}');
...
<section class="services-dots section-padding sub-bg radius-30 ">
    <div class="container">
        ...
        <div class="sec-head mb-30">
            <h6 class="sub-title main-color mb-15">
                {{ optional($page->designStrength->first())->design_title ?? 'Design Strength' }}
            </h6>
        ...
        <div class="col-12 ">
            <p>
                {!! nl2br(e(optional($page->designStrength->first())->design_text ?? '')) !!}
            </p>
        </div>
    </div>
</section>
