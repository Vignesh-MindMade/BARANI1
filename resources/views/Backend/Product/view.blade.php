                    ...
                    <table id="productsTable" class="table table-hover">
                        ...
                        <tbody>
                            @foreach ($products as $product)
                                <tr>
                                    <td>{{ $product->category->catagory ?? 'N/A' }}</td>
                                    <td>{{ $product->product_title }}</td>
                                    <!--<td>{{ Str::limit($product->product_description, 100) }}</td>-->
                                    <td>
                                        @if ($product->banner)
                                            <img src="{{ asset('uploads/products/' . $product->banner) }}" alt="Banner"
                                                class="img-thumbnail">
                                        @else
                                            No Image
                                        @endif
                                    ...
                                    <td>
                                        ...
                                        <form id="delete-form-{{ $product->id }}" method="POST"
                                            action="{{ route('product_view.destroy', $product->id) }}"
                                            style="display: none;">
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#add-new-product').on('click', function() {
                ...
                $('#productForm').attr('action', '{{ route('product_view.store') }}');
            ...
            $(document).on('click', '.edit-product', function() {
                ...
                $('#productForm').attr('action', '{{ route('product_view.update', ':id') }}'.replace(':id',
                    id));
                $('#method_field').val('POST');
                // Populate fields
    ...
    Swal.fire({
        ...
        //             if (imagesData.length > 0) {
        //                 $('#current-product-image').html('<strong>Current Images:</strong><br>');
        //                 const assetPath = "{{ asset('uploads/products/') }}";
