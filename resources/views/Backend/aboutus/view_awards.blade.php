...
<div class="section-divider"></div>
                ...
                <div class="row mt-4" id="addFormSection" style="display: none;">
                    <div class="col-12">
                        <div class="form-section">
                            ...
                            <form action="{{ route('view_awards.store') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="row g-4">
                                    <div class="col-md-6">
                                        ...
                                        <tbody>
                                            @foreach ($Histories as $history)
                                                <tr>
                                                    ...
                                                    <td>{{ Str::limit($history->view_awards_description ?? '', 80) }}</td>
                                                    <td>{{ $history->sort_id ?? '-' }}</td>
                                                    <td>
                                                        @if ($history->award_image)
                                                            <img src="{{ asset('images/' . $history->award_image) }}"
                                                                class="img-thumbnail"
                                                    ...
                                                    <td>
                                                        @if (!empty($history->images) && is_array($history->images))
                                                    ...
                                                    <td>
                                                        ...
                                                        <button class="btn btn-sm btn-outline-danger"
                                                            onclick="confirmDelete('{{ route('view_awards.destroy', $history->id) }}')">
                ...
                {{-- ───────────────────────────── --}}
                @foreach ($Histories as $history)
                    <div class="modal fade" id="editModal{{ $history->id }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-lg modal-dialog-scrollable">
                            <div class="modal-content">
                                ...
                                <form action="{{ route('view_awards.update', $history->id) }}" method="POST"
                                    ...
                                    <div class="modal-body">
                                        <div class="row g-3">
                                            ...
                                            <div class="col-12">
                                                ...
                                                <input class="form-control" name="title" value="{{ $history->title }}"
                                                    required>
                                            </div>

                                            <div class="col-12">
                                                <label class="form-label">Description</label>
                                                <textarea class="form-control" name="view_awards_description" rows="4">{{ $history->view_awards_description }}</textarea>
                                            </div>

                                            <div class="col-md-4">
                                                <label class="form-label">Sort ID</label>
                                                <input class="form-control" name="sort_id" type="number"
                                                    value="{{ $history->sort_id }}">
                                            </div>
                                            ...
                                            <div class="col-md-8">
                                                ...
                                                @if ($history->award_image)
                                                    <img src="{{ asset('images/' . $history->award_image) }}"
                                                        id="awardMainPreviewEdit{{ $history->id }}"
                                                        class="img-thumbnail mb-2" style="max-width:240px;">
                                                @endif

                                                <input type="file" name="award_image" class="form-control"
                                                    accept="image/*"
                                                    onchange="previewSingleImage(this, 'awardMainPreviewEdit{{ $history->id }}')">
                                            </div>
                                            ...
                                            <div class="col-12">
                                                ...
                                                <div class="multiple-images-container"
                                                    id="multipleImagesEdit{{ $history->id }}">

                                                    @if (is_array($history->images))
                                                        @foreach ($history->images as $img)
