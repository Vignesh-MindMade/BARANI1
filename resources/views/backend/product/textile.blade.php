...
<div class="container py-5">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('category.store') }}" method="POST">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('productstextile') }}" class="btn btn-secondary">Cancel</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="card shadow-sm mb-5">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('productstextile.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <h5 class="section-header">Product Details</h5>
                <div class="row">
                    <div class="col-md-6 mb-4">
                        <label for="product_textile_id" class="form-label">Select Category <span class="text-danger">*</span></label>
                        <select name="product_textile_id" id="product_textile_id" class="form-select @error('product_textile_id') is-invalid @enderror" required>
                ...
                <table class="table table-hover table-bordered">
                    ...
                    <tbody>
                        @forelse ($products as $product)
                            <tr>
                                <td>{{ $product->id }}</td>
                                <td>{{ $product->productTextile->catagory_name }}</td>
                                ...
                                <td>
                                    <a href="{{ route('productstextile.edit', $product->id) }}" class="btn btn-sm btn-warning me-2">Edit</a>
                                    <form action="{{ route('productstextile.destroy', $product->id) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure?')">Delete</button>
                                    </form>
