...
//       logo.attr("src", "{{url('assets/frontend/imgs/logo-light.png')}}");
//     } else {
...
$(window).on("scroll", function () {
    ...
    if (!logo.length || !window.appConfig) return;

    if (bodyScroll > 300) {
        navbar.addClass("nav-scroll");

        if (window.appConfig.logoDark) {
            logo.attr("src", window.appConfig.logoDark);
        }

    } else {
        navbar.removeClass("nav-scroll");

        if (window.appConfig.logoLight) {
            logo.attr("src", window.appConfig.logoLight);
        }
    }
});
 ...
 // =============================================
// Parallax Swiper with Video Reset on Slide Change
// =============================================
...
var parallaxSliderOptions = {
    ...
    // We will handle video play/reset manually — disable native autoplay if needed
    // autoplay: false,   // ← you can try this if browser autoplay policies cause issues
    
    on: {
        init: function () {
            var swiper = this;
            
            // Set parallax for each slide
            ...
            var firstVideo = swiper.slides[swiper.activeIndex].querySelector('video');
            if (firstVideo) {
                firstVideo.currentTime = 0;
                firstVideo.play().catch(e => console.log("Autoplay prevented:", e));
            }

                    // Defer progress bar start — loop mode repositions slides after init,
            // which fires slideChangeTransitionStart and resets the bar.
            // A small timeout lets Swiper finish its loop setup first.
            var self = this;
            setTimeout(function () {
                updateProgressBar.call(self);
            }, 1);
        },

        slideChangeTransitionStart: function () {
            // Pause ALL videos when transition begins (prevents overlap)
            document.querySelectorAll('.swiper-slide video').forEach(function(video) {
                video.pause();
            });

            // Reset progress bars
            $(".swiper-pagination-bullet .progress").stop(true, true).css("width", "0%");
        },
        ...
        slideChangeTransitionEnd: function () {
            ...
            // Update progress bar animation for the active bullet
            updateProgressBar.call(swiper);
        },
...
function updateProgressBar() {
    ...
    $(".swiper-pagination-bullet .progress").css("width", "0%");
...
var parallaxSlider = new Swiper(".slider-prlx .parallax-slider", parallaxSliderOptions);
...
$(function () {
  ...
  $(document).ready(function () {
    $('[data-carousel="swiper"]').each(function () {
      ...
      if ($(this).hasClass("swiper6")) {
        conf = {
          ...
          autoplay: {
            ...
            disableOnInteraction: false,
            pauseOnMouseEnter: true,
            reverseDirection: true, // ðŸ‘ˆ left â†’ right
          },
          slidesPerView: "auto",
          freeMode: true,
          freeModeMomentum: false,
      ...
      if (prev) conf.navigation = conf.navigation || {};
      if (prev) conf.navigation.prevEl = "#" + prev;
      if (next) conf.navigation = conf.navigation || {};
      if (next) conf.navigation.nextEl = "#" + next;
      if (pagination)
        conf.pagination = { el: "#" + pagination, clickable: true };
      ...
      if (containe) {
        ...
        try {
          ...
          console.error(
            "Swiper initialization failed for " + initID + ": ",
            error
          );
        }
      }
